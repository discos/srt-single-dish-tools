...
class TestInspect(object):
    ...
    def test_inspect_observations02(self):
        assert self.groups["CCB,SARDARA"]["W44"]["Obs0"]["Cal"] == ["3C48_0.1_CCB", "3C295_0.2_CCB"]

    def test_inspect_observations03(self):
        assert self.groups["KKG,SARDARA"]["3C157"]["Obs0"]["Src"] == ["3C157_0.3_KKG", "3C157_0.4_KKG"]

    def test_inspect_observations04(self):
        assert self.groups["KKG,SARDARA"]["3C157"]["Obs0"]["Cal"] == ["3C48_0.5_KKG", "3C48_0.6_KKG"]
    ...
    def test_inspect_observations06(self):
        assert self.groups["CCB,SARDARA"]["W44"]["Obs1"]["Cal"] == ["3C48_0.8_CCB", "3C48_0.9_CCB"]
    ...
    def test_inspect_observations08(self):
        assert self.groups["KKG,SARDARA"]["W44"]["Obs0"]["Cal"] == ["3C48_0.6_KKG", "3C295_1.1_KKG", "3C48_1.2_KKG"]

    def test_inspect_observations09(self):
        dump_config_files(self.info)
        config = ConfigParser()
        config.read("CCB_SARDARA_W44_Obs0.ini")
        assert config.get("analysis", "calibrator_directories").strip().split("\n") == \
